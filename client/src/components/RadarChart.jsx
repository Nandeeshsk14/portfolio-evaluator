...
function RadarChart({ scores, compareScores = null, primaryLabel = 'Your Score', compareLabel = null }) {
  if (!scores) return null;

  const labels = ['Activity', 'Code Quality', 'Diversity', 'Community', 'Hiring Ready'];
  const values = [scores.activity, scores.codeQuality, scores.diversity, scores.community, scores.hiringReady];
  ...
  if (compareScores) {
    datasets.push({
      ...
      data: [compareScores.activity, compareScores.codeQuality, compareScores.diversity, compareScores.community, compareScores.hiringReady],
